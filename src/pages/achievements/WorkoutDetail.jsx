// src/pages/achievements/WorkoutDetail.js
...
const AchievementWorkoutDetail = () => {
  ...
  const fetchWorkoutDetail = async () => {
    try {
      const response = await axiosInstance.get(`/user-daily-routine/workout/${id}`);
  ...
  const fetchLikes = async () => {
    try {
      const response = await axiosInstance.get(`/likes/`, {
        params: { targetId: id, targetType: 'Workout' }
      });
  ...
  const fetchComments = async (page = 1) => {
    ...
    try {
      const response = await axiosInstance.get(`/comments/`, {
        params: { targetId: id, targetType: 'Workout', page, limit: 5 }
      });
  ...
  const isOwner = workout?.userId?._id === user?._id;
              ...
              <div>
                <p className="font-medium text-sm" style={{ color: 'var(--theme-text)' }}>
                  ...
                  {comment.userId?._id === user?._id && (
                    <span className="ml-2 text-xs px-2 py-0.5 rounded-full" style={{ background: 'rgba(59, 130, 246, 0.1)', color: 'var(--theme-primary)' }}>
                      You
                    </span>
                  )}
                </p>
            ...
            <div className="mt-2">
              ...
              <div className="flex justify-end space-x-2 mt-2">
                ...
                <button
                  ...
                  className="px-3 py-1.5 rounded-lg text-sm"
                  style={{ background: 'rgba(59, 130, 246, 0.1)', color: 'var(--theme-primary)' }}
                >
                  Save
                </button>
              </div>
      ...
      <div className="max-w-3xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center space-x-4">
            ...
            <h1 className="text-2xl font-bold gradient-text">Workout Achievement</h1>
          </div>
          {isOwner && (
            <div className="px-3 py-1.5 rounded-full text-sm" style={{ background: 'rgba(59, 130, 246, 0.1)', color: 'var(--theme-primary)' }}>
              Your Workout
            </div>
          )}
        </div>
        ...
        <GlassCard className="p-6 mb-4">
          ...
          <div className="flex items-center justify-between mb-6 p-4 rounded-xl" style={{ background: 'rgba(255,255,255,0.03)' }}>
            <div 
              className={`flex items-center space-x-3 ${workout.userId?._id === user?._id ? 'cursor-pointer' : ''}`}
              onClick={() => navigate(workout.userId?._id === user?._id ? '/profile' : `/user/${workout.userId?._id}`)}
            >
              ...
              <div>
                ...
                <p className="text-lg font-semibold" style={{ color: 'var(--theme-text)' }}>
                  {workout.userId?.userName || 'Anonymous User'}
                  {workout.userId?._id === user?._id && (
                    <span className="ml-2 text-sm px-2 py-0.5 rounded-full" style={{ background: 'rgba(59, 130, 246, 0.1)', color: 'var(--theme-primary)' }}>
                      You
                    </span>
                  )}
                </p>
            ...
            {workout.userId?._id !== user?._id && (
              <button
                onClick={() => navigate(`/user/${workout.userId?._id}`)}
          ...
          <div className="mb-6">
            <div className="flex items-center space-x-2 mb-2">
              ...
              <span className="text-sm font-medium px-3 py-1 rounded-full" style={{ background: 'rgba(255,255,255,0.05)' }}>
                {workout.category}
              </span>
            </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mb-6">
            <div className="p-4 rounded-xl text-center" style={{ background: 'rgba(255,255,255,0.03)' }}>
              <Layers size={24} className="mx-auto mb-2" style={{ color: 'var(--theme-primary)' }} />
            ...
            <div className="p-4 rounded-xl text-center" style={{ background: 'rgba(255,255,255,0.03)' }}>
              <Target size={24} className="mx-auto mb-2" style={{ color: 'var(--theme-success)' }} />
            ...
            <div className="p-4 rounded-xl text-center" style={{ background: 'rgba(255,255,255,0.03)' }}>
              <Dumbbell size={24} className="mx-auto mb-2" style={{ color: 'var(--theme-warning)' }} />
          ...
          {workout.notes && (
            <div className="mb-6 p-4 rounded-xl" style={{ background: 'rgba(255,255,255,0.03)' }}>
              <p className="text-sm mb-2" style={{ color: 'var(--theme-textMuted)' }}>Notes</p>
              <p className="text-lg italic" style={{ color: 'var(--theme-textSecondary)' }}>"{workout.notes}"</p>
            </div>
          )}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="p-4 rounded-xl" style={{ background: 'rgba(255,255,255,0.03)' }}>
              ...
              <div className="flex items-center space-x-2">
                <div className="w-3 h-3 rounded-full" style={{ backgroundColor: getStatusColor(workout.status) }} />
                <span className="text-lg font-medium" style={{ color: 'var(--theme-text)' }}>{workout.status}</span>
              </div>
            </div>
          ...
          <div className="mt-4 flex justify-center">
            <div className="flex items-center space-x-2 px-4 py-2 rounded-full" style={{ background: 'rgba(59, 130, 246, 0.1)' }}>
            ...
            {replyTo && (
              <div className="flex items-center justify-between mt-2 px-3 py-2 rounded-lg" style={{ background: 'rgba(255,255,255,0.02)' }}>
                <span className="text-sm" style={{ color: 'var(--theme-textSecondary)' }}>
                  Replying to <span style={{ color: 'var(--theme-primary)' }}>@{replyTo.userId?.userName}</span>
                </span>
                <button
                  type="button"
                  onClick={() => {
...
export default AchievementWorkoutDetail;