// src/pages/nutrition/EditNutrition.js
...
const EditNutrition = () => {
  ...
  const fetchNutrition = async () => {
    try {
      ...
      const alertTime = nutrition.alertTime 
        ? new Date(nutrition.alertTime).toISOString().slice(0, 16)
        : '';
      ...
      setFormData({
        ...
        fats: nutrition.fats || '',
        alertTime: alertTime,
        alertRecurring: nutrition.alertRecurring || 'once'
      });

      // Enable alert toggle if there's an alert time set
      setEnableAlert(!!nutrition.alertTime);
    } catch (error) {
      console.error('Error fetching nutrition:', error);
      toast.error('Failed to load nutrition entry');
      navigate('/nutrition');
    } finally {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axiosInstance.put(`/nutrition/update/nutrition/${id}`, submitData);
      toast.success(response.data.message);
      navigate('/nutrition');
    } catch (error) {
      console.error('Update error:', error);
      toast.error(error.response?.data?.message || 'Failed to update nutrition entry');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="min-h-screen p-6" style={{ background: 'var(--theme-bg)' }}>
      <div className="max-w-3xl mx-auto">
        ...
        <div className="flex items-center space-x-4 mb-6">
          ...
          <h1 className="text-2xl font-bold gradient-text">Edit Food Entry</h1>
        </div>
